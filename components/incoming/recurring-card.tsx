...
export function RecurringCard({ expense, onEdit }: RecurringCardProps) {
  ...
  return (
    <Card className="relative overflow-hidden bg-card border-card-border p-5 transition-all hover:bg-card-hover hover:border-card-hover group">
      ...
      <div className="flex justify-between items-start mb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-text-muted">{expense.category?.name || 'Uncategorized'}</p>
          </div>
        </div>

        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <button className="text-text-muted hover:text-text-primary transition-colors opacity-0 group-hover:opacity-100 p-1">
              <MoreHorizontal className="h-4 w-4" />
            </button>
          </DropdownMenuTrigger>
          <DropdownMenuContent align="end" className="bg-card border-card-border text-text-primary">
            <DropdownMenuItem onClick={() => onEdit(expense)} className="hover:bg-background-secondary cursor-pointer">
              <Edit2 className="h-4 w-4 mr-2" />
              Edit
            </DropdownMenuItem>
            <DropdownMenuItem 
              onClick={handleDelete} 
              className="text-red-500 hover:bg-background-secondary hover:text-red-400 cursor-pointer"
            >
      ...
      <div className="flex items-end justify-between">
        <div>
          <p className="text-2xl font-bold text-text-primary mb-1">
            {Number(expense.amount).toFixed(2)} <span className="text-sm font-normal text-text-muted">PLN</span>
          </p>
        ...
        <div className="text-right">
           <div className={cn(
             ...
             daysLeft <= 3 ? "bg-red-500/10 text-red-500 border border-red-500/20" : 
             daysLeft <= 7 ? "bg-amber-500/10 text-amber-500 border border-amber-500/20" :
